...
class CreatePlanListTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('plan_list', function (Blueprint $table) {
            ...
            $table->enum('process',['none','sole','inject_mold','box_lable','put_in_storage','out_of_storage','delivery'])
                ->comment('计划单进程')->default('none');
            $table->decimal('delivery_num',10,2)->comment('发货数量')->nullable()->default(0);
