...
class CreateTransitStorageIn extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('transit_storage_in', function (Blueprint $table) {
            ...
            $table->enum('type',['pqcjmpjgck','pqcjddbcpjgck','wpddjgck','zcpck','bfck','wppqmpck','wppqddbcpck','other'])
                ->comment('出库类型')
                ->default('pqcjmpjgck');
            ...
            $table->string('spec')->comment('规格明细')->nullable();
            $table->unsignedBigInteger('personnel_id')->comment('员工加工');
            $table->string('personnel_name')->comment('员工加工')->nullable();
            $table->text('remark')->comment('描述')->nullable();

            $table->decimal('all_num',10,2)->comment('出库数量')->nullable()->default(0);
            $table->softDeletes();
            $table->timestamps();
        });
    }
    ...
    {
        Schema::dropIfExists('transit_storage_in');
    }
}
