...
use App\Models\MoldInformationVoidLog;
use Carbon\Carbon;
...
class MoldInformationVoidChange extends Form
{
    // 增加一个自定义属性保存用户ID
    protected $id;
    protected $data;
    ...
    {
        $this->id = $id;
        $this->data = MoldInformation::find($id);
        parent::__construct();
    }
    /**
    ...
    public function handle(array $input)
    {
        ...
        $info = MoldInformation::find($id);

        DB::beginTransaction(); //开启事务
        try{
            $void_log_model = new MoldInformationVoidLog();
            $now = Carbon::now();
            $void_log_model->insert([
                'mold_information_id'=>$info->id,
