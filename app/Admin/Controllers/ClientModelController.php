...
class ClientModelController extends AdminController
{
    /**
     * Make a grid builder.
     *
    ...
    protected function grid()
    {
        return Grid::make(new ClientModel(), function (Grid $grid) {
            ...
            $grid->filter(function (Grid\Filter $filter) {
                $filter->equal('client_model_name');
            });
        });
    }
    ...
    protected function iFrameGrid()
    {
        ...
        $grid->actions(function (Grid\Displayers\Actions $actions) {
            ...
            {client_model_id:'.$rowArray['id'].',
            client_model:\''.$rowArray['client_model_name'].'\'
            })" > 选择</a>
<script>
...
function chooseThis(data) {
    ...
    $(window.parent.document).find("div[name=client_model_id]").empty().text(data.client_model);
...
');
        ...
        $grid->filter(function (Grid\Filter $filter) {
            $filter->panel();
            $filter->expand();
            $filter->like('client_model_name')->width(6);
        });
    ...
    {
        $q      = $request->get('q');
        $query =  ClientModel::where('status','1');
        if($q){
            $query->where('client_model_name', 'like',$q);
        }
        $result =$query->orderBy('created_at','desc')->get();
        // 把查询出来的结果重新组装成 Laravel-Admin 需要的格式
        $result = $result->map(function (ClientModel $data) {
            return ['id' => $data->id, 'text' => $data->client_model_name];
        });
        return $result;
